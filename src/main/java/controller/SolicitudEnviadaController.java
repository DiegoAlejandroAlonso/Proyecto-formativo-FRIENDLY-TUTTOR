...
import model.SolcitudenviadaDao;
import model.SolcitudenviadaVO;
 ...
 */
@WebServlet("/SolicitudEnviadaController")
public class SolicitudEnviadaController extends HttpServlet {
	private String host;
	...
	SolcitudenviadaDao sdao=new SolcitudenviadaDao();
     ...
     */
    public SolicitudEnviadaController() {
        super();
		...
		try {
			if(accion!=null) {
				...
				case "listarSolicitudesEnviadas":
					...
					listarSolicitudesEnviadas(request,response);
				break;
	...
	private void listarSolicitudesEnviadas(HttpServletRequest request, HttpServletResponse response) {
		// TODO Auto-generated method stub
		
		System.out.println("Entramos al método listar");
		
		try {
			List solicitudes=sdao.listarSolicitudEnviada();
			request.setAttribute("solicitudEnviada",solicitudes);
	...
	private void add(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			sdao.registrar(s);
			response.sendRedirect("SolicitudEnviadaController?accion=listarSolicitudesEstudiante");
	...
	private void add2(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			sdao.registrar(s);
			response.sendRedirect("SolicitudEnviadaController?accion=listarSolicitudesTutorCorreo");
			System.out.println("solicitud registrada");
			System.out.println("" + s.getNombre() + s.getDescripcion() + s.getUsuario()
					+ s.isEstado());
		} catch (Exception e) {
	...
	private void ver(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			request.setAttribute("solicitudEnviada", s);
			request.getRequestDispatcher("views/solicitudEnviada-edit.jsp").forward(request, response);
	...
	private void edit(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			sdao.edit(s);
			response.sendRedirect("SolicitudEnviadaController?accion=listarSolicitudesTutorCorreo");
			System.out.println("Solicitud enviada cambiada");
		} catch (Exception e) {

			System.out.println("Error al cambiar la solicitud enviada");
		}
